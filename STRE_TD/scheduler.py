    ...
    def construire(self):
        ...
        for j in self._jobs:
            ...
            while workTime > 0:
                ...
                free_time = self._schedule[i+1][1] - \
                    self._schedule[i][1] - self._schedule[i][2]
                if free_time > 0:
                    if free_time >= workTime:
                        self._schedule.insert(
                            i+1, [j[0],  self._schedule[i][1] + self._schedule[i][2], workTime])
                    else:
                        self._schedule.insert(
                            i+1, [j[0],  self._schedule[i][1] + self._schedule[i][2], free_time])
                        workTime -= free_time
